import { useEffect } from 'react'
import { useNavigate, useLocation } from 'react-router'
import { motion } from 'motion/react'
import { Calendar, Sparkles, CheckCircle2 } from 'lucide-react'
import { Button } from '../components/ui/button'
import { useAuth } from '../context/AuthContext'

export function Auth() {
  const { user, loading, signInWithGoogle } = useAuth()
  const navigate = useNavigate()
  const location = useLocation()

  // Where to send the user after sign-in (default: /calendar)
  const from = (location.state as { from?: string })?.from ?? '/calendar'

  useEffect(() => {
    if (!loading && user) {
      navigate(from, { replace: true })
    }
  }, [user, loading, from, navigate])

  return (
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col">
      {/* Header */}
      <header className="bg-white border-b border-neutral-200 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center gap-2">
          <div className="size-8 bg-neutral-900 rounded-lg flex items-center justify-center">
            <Calendar className="size-5 text-white" />
          </div>
          <span className="text-lg font-semibold text-neutral-900">Canvas2Calendar</span>
        </div>
      </header>

      {/* Main */}
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="w-full max-w-md"
        >
          {/* Card */}
          <div className="bg-white rounded-2xl border border-neutral-200 shadow-lg overflow-hidden">
            {/* Top gradient */}
            <div className="bg-gradient-to-br from-neutral-900 to-neutral-700 p-8 text-white text-center">
              <div className="size-16 bg-white/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Sparkles className="size-8 text-white" />
              </div>
              <h1 className="text-2xl font-semibold mb-2">Welcome to Canvas2Calendar</h1>
              <p className="text-neutral-300 text-sm">
                Sign in to save your tasks, schedules, and sync to Google Calendar.
              </p>
            </div>

            <div className="p-8">
              {/* Feature list */}
              <ul className="space-y-3 mb-8">
                {[
                  'Import assignments from Canvas',
                  'AI-powered task decomposition',
                  'Drag-and-drop weekly planner',
                  'One-click Google Calendar sync',
                ].map((feat) => (
                  <li key={feat} className="flex items-center gap-3 text-sm text-neutral-700">
                    <CheckCircle2 className="size-4 text-green-600 flex-shrink-0" />
                    {feat}
                  </li>
                ))}
              </ul>

              {/* Sign-in button */}
              <Button
                onClick={signInWithGoogle}
                size="lg"
                className="w-full bg-neutral-900 hover:bg-neutral-800 text-white h-12 flex items-center gap-3"
              >
                {/* Google "G" logo SVG */}
                <svg className="size-5" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                Continue with Google
              </Button>

              <p className="text-xs text-neutral-500 text-center mt-4">
                You'll be asked to approve Google Calendar access so tasks can sync
                directly to your calendar.
              </p>
            </div>
          </div>

          <p className="text-center text-xs text-neutral-400 mt-6">
            Built for Berkeley SCET students · Free during beta
          </p>
        </motion.div>
      </div>
    </div>
  )
}
