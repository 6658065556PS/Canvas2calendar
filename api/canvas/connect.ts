...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // 2. Build Canvas client and verify token by fetching courses
  let courses
  try {
    const canvas = createCanvasClient({ accessToken: canvasToken })
    courses = await canvas.getCourses()
  } catch (err) {
    console.error('[connect] Canvas API error:', err)
  ...
  if (deleteErr) {
    console.error('[connect] Failed to delete stale assignments:', deleteErr.message)
    return res.status(500).json({ error: 'Failed to clear old assignments before sync' })
  }
  ...
  for (const course of courses) {
    ...
    try {
      ...
      console.warn(`[connect] Skipping course ${course.name}:`, err)
      continue
    }
